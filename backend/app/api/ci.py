...
async def get_ci_run_status(
    ...
    hist_cost_per_test = sum((float(t.cost_usd) if t.cost_usd else 0.0) for t in hist_tests) / len(hist_tests)
    ...
    if curr_cost_per_test > hist_cost_per_test * 1.2:
        failed = True
        justification_lines.append(f"Cost increased by >20% (Historical avg: ${hist_cost_per_test:.6f}, Current avg: ${curr_cost_per_test:.6f}).")
    else:
        justification_lines.append(f"Cost is within acceptable limits (Current avg: ${curr_cost_per_test:.6f}).")
        
    if avg_score < hist_avg_score:
        failed = True
        justification_lines.append(f"Semantic precision dropped (Historical avg: {hist_avg_score:.2f}, Current avg: {avg_score:.2f}).")
    else:
        justification_lines.append(f"Semantic precision is stable or improved (Current avg: {avg_score:.2f}).")
