...
const PromptAnalytics: React.FC<PromptAnalyticsProps> = ({
  promptId,
  ...
  const bestLatency =
    successfulVersions.length > 0
      ? [...successfulVersions].sort((a, b) => a.avg_latency - b.avg_latency)[0]
      : null
  const bestCost =
    successfulVersions.length > 0
      ? [...successfulVersions].sort((a, b) => a.avg_cost - b.avg_cost)[0]
      : null

  const renderBarChart = (
    title: string,
    subtitle: string,
    field: keyof EvolutionData,
    unit: string,
    color: string,
    lowerIsBetter = true
  ) => {
    ...
    if (activeHasData && prevData) {
      ...
      if (previousVal > 0) {
        ...
        trendNode = (
          <span
            className={`trend-tag ${isBetter ? 'better' : isWorse ? 'worse' : 'neutral'}`}
          >
            {diffPercent > 0 ? '+' : ''}
            {diffPercent.toFixed(1)}% vs v{prevData.version}
          </span>
        )
      }
    }
            ...
            return (
              <div key={i} className="bar-wrapper">
                <div className="bar-value-popup">
                  {hasTests ? val.toFixed(unit === '$' ? 4 : 1) + unit : 'N/A'}
                </div>
                <div
      ...
      <div className="analytics-summary-cards">
        <div className="insight-card">
          ...
          <span className="value">
            {activeHasSuccess
              ? `${activeData.avg_latency.toFixed(0)}ms`
              : '---'}
          </span>
          <span className="sub">
            {activeHasSuccess &&
            bestLatency &&
            activeData.avg_latency <= bestLatency.avg_latency
              ? 'Recorde de velocidade!'
              : bestLatency
                ? `Melhor global: ${bestLatency.avg_latency.toFixed(0)}ms`
                : 'Sem recordes ainda'}
          </span>
        </div>
        <div className="insight-card">
          ...
          <span className="value">
            {activeHasSuccess ? `$${activeData.avg_cost.toFixed(4)}` : '---'}
          </span>
          <span className="sub">
            {activeHasSuccess &&
            bestCost &&
            activeData.avg_cost <= bestCost.avg_cost
              ? 'Versão mais econômica'
              : bestCost
                ? `Mínimo global: $${bestCost.avg_cost.toFixed(4)}`
                : 'Sem recordes ainda'}
          </span>
        </div>
        <div className="insight-card">
          ...
          <span className="value">
            {activeHasData
              ? `${((activeData.success_count / activeData.test_count) * 100).toFixed(1)}%`
              : '---'}
          </span>
          <span className="sub">
            {activeHasData
              ? `${activeData.success_count} sucessos em ${activeData.test_count} testes`
              : 'Aguardando testes'}
          </span>
        </div>
        <div className="insight-card">
          ...
          <span className="value">
            {data.reduce((acc, curr) => acc + curr.test_count, 0)} testes
          </span>
          ...
          <div className="chart-container">
            {data.map((v, i) => {
              ...
              const successRate = hasData
                ? (v.success_count / v.test_count) * 100
                : 0
              ...
              return (
                <div key={i} className="bar-wrapper">
                  <div className="bar-value-popup">
                    {hasData ? successRate.toFixed(0) + '%' : 'N/A'}
                  </div>
                  <div
                  ...
                  >
                    {hasData && v.fail_count > 0 && (
                      <div
                        ...
                        style={{
                          height: `${(v.fail_count / (v.success_count + v.fail_count)) * 100}%`,
                        }}
                      />
                    )}
                  </div>
          ...
          <tbody>
            {[...data].reverse().map((v) => (
              ...
              >
                ...
                <td>{v.test_count}</td>
                <td>
                  {v.avg_latency > 0 ? `${v.avg_latency.toFixed(0)}ms` : '---'}
                </td>
