...
const Playground: React.FC = () => {
  ...
  const handleExecute = async () => {
    if (!promptContent.trim()) {
      setAlert({ type: 'error', message: 'O conteúdo do prompt não pode ser vazio.' });
      return;
    }
    ...
    try {
      ...
      const updatedResults = [null, null, null] as (PlaygroundColumnResult | null)[];
      ...
      setAlert({
        type: 'error',
        message: err instanceof Error ? err.message : 'Erro ao executar o teste paralelo.',
      });
    } finally {
      setExecuting(false);
    }
  ...
  const handleSaveAsVersion = async () => {
    ...
    try {
      ...
      setAlert({
        type: 'error',
        message: err instanceof Error ? err.message : 'Falha ao salvar o prompt como nova versão.',
      });
    } finally {
      setSavingIndex(null);
    }
  ...
  if (loading) return <Loading message="Inicializando ambiente do Playground..." />;
  if (error) return <div className="playground-error"><Alert type="error" message={error} /></div>;
  if (!prompt) return <div className="playground-error"><Alert type="error" message="Prompt não encontrado." /></div>;
      ...
      <div className="playground-container">
        <div className="playground-header-bar">
          <div className="back-nav">
            <button className="btn-back" onClick={() => navigate(`/prompts/${id}`)}>
              ← Voltar para {prompt.name}
            </button>
          </div>
          <div className="title-section">
            <h1>Playground Multi-Provedor A/B/C</h1>
            <p className="subtitle">Execute e compare a performance de diferentes modelos em tempo real</p>
          </div>
        </div>
        ...
        <div className="playground-workspace">
          ...
          <div className="playground-editor-panel">
            <div className="panel-section">
              ...
              <small className="help-text">
                Dica: O SandboxAI renderiza este prompt usando <code>Jinja2</code>. Use <code>{"{{variavel}}"}</code> para colmatar campos dinâmicos.
              </small>
            </div>
          ...
          <div className="playground-comparison-view">
            {configs.map((config, index) => {
              ...
              const isAvailable = providerStatus[config.provider]?.available ?? true;
              const statusReason = providerStatus[config.provider]?.reason ?? '';
              const borderThemeColor = PROVIDER_COLORS[config.provider] || 'var(--primary-color)';

              return (
                <div 
                  key={index} 
                  className="comparison-column"
                  style={{ '--column-theme-color': borderThemeColor } as React.CSSProperties}
                >
                  {/* Column Config Header */}
                  <div className="column-config-header">
                    <span className="column-label">Coluna {String.fromCharCode(65 + index)}</span>
                    <div className="config-inputs">
                      ...
                      >
                        {(DEFAULT_MODELS[config.provider] || []).map((modelName) => (
                    ...
                    {executing && !result ? (
                      ...
                      <div className="column-result-content">
                        ...
                        <div className="result-badges-row">
                          ...
                          {result.is_correct !== null && result.is_correct !== undefined && (
                            <span className={`validation-pill ${result.is_correct ? 'pass' : 'fail'}`}>
                              {result.is_correct ? '✅ PASS' : '❌ FAIL'} ({(result.score || 0).toFixed(2)})
                            </span>
                          )}
                        </div>
                        ...
                        {result.status === 'completed' && (
                          <div className="column-metrics-grid">
                            <div className="metric-box">
                              ...
                              <span className="metric-val">{result.latency_ms.toFixed(0)}ms</span>
                            </div>
                            <div className="metric-box">
                              <span className="metric-lbl">Tokens</span>
                              <span className="metric-val">{result.tokens_used}</span>
                            </div>
                            <div className="metric-box">
                              <span className="metric-lbl">Custo</span>
                              <span className="metric-val">${result.cost_usd.toFixed(6)}</span>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="output-scroll-box">
                          {result.status === 'completed' ? (
                            <pre className="output-raw-text">{result.output}</pre>
                          ) : (
